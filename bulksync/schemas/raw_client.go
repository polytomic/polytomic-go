// Code generated by Fern. DO NOT EDIT.

package schemas

import (
	context "context"
	http "net/http"

	polytomic "github.com/polytomic/polytomic-go/v25"
	bulksync "github.com/polytomic/polytomic-go/v25/bulksync"
	core "github.com/polytomic/polytomic-go/v25/core"
	internal "github.com/polytomic/polytomic-go/v25/internal"
	option "github.com/polytomic/polytomic-go/v25/option"
)

type RawClient struct {
	baseURL string
	caller  *internal.Caller
	options *core.RequestOptions
}

func NewRawClient(options *core.RequestOptions) *RawClient {
	return &RawClient{
		options: options,
		baseURL: options.BaseURL,
		caller: internal.NewCaller(
			&internal.CallerParams{
				Client:      options.HTTPClient,
				MaxAttempts: options.MaxAttempts,
			},
		),
	}
}

func (r *RawClient) List(
	ctx context.Context,
	// Unique identifier of the bulk sync.
	id string,
	request *bulksync.SchemasListRequest,
	opts ...option.RequestOption,
) (*core.Response[*polytomic.ListBulkSchemaEnvelope], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://app.polytomic.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/api/bulk/syncs/%v/schemas",
		id,
	)
	queryParams, err := internal.QueryValues(request)
	if err != nil {
		return nil, err
	}
	if len(queryParams) > 0 {
		endpointURL += "?" + queryParams.Encode()
	}
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *polytomic.ListBulkSchemaEnvelope
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(bulksync.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*polytomic.ListBulkSchemaEnvelope]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) Patch(
	ctx context.Context,
	// Unique identifier of the bulk sync.
	id string,
	request *bulksync.UpdateBulkSyncSchemasRequest,
	opts ...option.IdempotentRequestOption,
) (*core.Response[*polytomic.UpdateBulkSyncSchemasEnvelope], error) {
	options := core.NewIdempotentRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://app.polytomic.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/api/bulk/syncs/%v/schemas",
		id,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *polytomic.UpdateBulkSyncSchemasEnvelope
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPatch,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(bulksync.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*polytomic.UpdateBulkSyncSchemasEnvelope]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) Get(
	ctx context.Context,
	// Unique identifier of the bulk sync.
	id string,
	// Source-side schema identifier.
	schemaID string,
	opts ...option.RequestOption,
) (*core.Response[*polytomic.BulkSchemaEnvelope], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://app.polytomic.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/api/bulk/syncs/%v/schemas/%v",
		id,
		schemaID,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *polytomic.BulkSchemaEnvelope
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodGet,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(bulksync.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*polytomic.BulkSchemaEnvelope]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) Update(
	ctx context.Context,
	// Unique identifier of the bulk sync.
	id string,
	// Source-side schema identifier.
	schemaID string,
	request *bulksync.UpdateBulkSchema,
	opts ...option.IdempotentRequestOption,
) (*core.Response[*polytomic.BulkSchemaEnvelope], error) {
	options := core.NewIdempotentRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://app.polytomic.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/api/bulk/syncs/%v/schemas/%v",
		id,
		schemaID,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	headers.Add("Content-Type", "application/json")
	var response *polytomic.BulkSchemaEnvelope
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPut,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Request:         request,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(bulksync.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*polytomic.BulkSchemaEnvelope]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}

func (r *RawClient) Cancel(
	ctx context.Context,
	// The bulk sync ID.
	id string,
	// The schema ID to cancel for the bulk sync.
	schemaID string,
	opts ...option.IdempotentRequestOption,
) (*core.Response[*polytomic.CancelBulkSyncResponseEnvelope], error) {
	options := core.NewIdempotentRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
		"https://app.polytomic.com",
	)
	endpointURL := internal.EncodeURL(
		baseURL+"/api/bulk/syncs/%v/schemas/%v/cancel",
		id,
		schemaID,
	)
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *polytomic.CancelBulkSyncResponseEnvelope
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
			Method:          http.MethodPost,
			Headers:         headers,
			MaxAttempts:     options.MaxAttempts,
			BodyProperties:  options.BodyProperties,
			QueryParameters: options.QueryParameters,
			Client:          options.HTTPClient,
			Response:        &response,
			ErrorDecoder:    internal.NewErrorDecoder(bulksync.ErrorCodes),
		},
	)
	if err != nil {
		return nil, err
	}
	return &core.Response[*polytomic.CancelBulkSyncResponseEnvelope]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
}
