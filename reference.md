...
**resync:** `*bool` — Deprecated: use resync_mode instead. Equivalent to resync_mode=rebuild.
...
**test:** `*bool` — When true, runs a test execution that validates the configuration without writing to the destination. Mutually exclusive with resync and resync_mode.
...
**fields:** `[]*polytomic.V4UserFieldRequest` — Fields to create or update on the schema. Existing user-defined fields with the same field_id are replaced.
...
**type_:** `*string` — Target object type to query (e.g. schema name). When supplied, the response is narrowed to objects matching this type.
...
**search:** `*string` — Substring filter applied to target object names. Combine with type to browse large schemas.
